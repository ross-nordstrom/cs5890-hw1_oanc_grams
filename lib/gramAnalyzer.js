...
function analyzeSentencesAndWords(cacheDir, files) {
    var cachePath = [cacheDir, 'step3_sentenceAndWordInfo.json'].join('/');
    ...
    var sentenceStatsByFile = files.map(statsForFile);
...
function bigramCounts(listsOfWords) {
    var bigramWords = _.map(listsOfWords, function produceBigramsAsWords(acc, words) {
        var bigrams = nGrams.bigrams(words);
        return bigrams.map(serializeBigram); // bigramsAsWords
...
function serializeBigram(word1, word2) {
