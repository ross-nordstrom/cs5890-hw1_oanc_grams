 ...
 */
function statsForFile(file, idx) {
 ...
 */
function combineSentenceStats(a, b) {
    ...
    var sentenceStats = {
        ...
        //unigrams: aggregateWordCounts(a.unigrams, b.unigrams),
        //bigrams: aggregateWordCounts(a.bigrams, b.bigrams)
    };
    return sentenceStats;
}

/**
 * Add up (accumulate) multiple word count objects (each of format {word1: count1, word2: count2, ...})
 * @param {Object...} words - The words to aggregate together
 * @return {*}
 */
function aggregateWordCounts(/*words*/) {
    var wordCounts = _.toArray(arguments);
    // Produce tuples for each key/value pair and then flatten them all to the same level
    var countTuples = _.union.apply(null, wordCounts.map(_.pairs));
    return _.reduce(countTuples, function combineWordCounts(combinedWordCount, tuple) {
        combinedWordCount[tuple[0]] = (combinedWordCount[tuple[0]] || 0) + tuple[1];
